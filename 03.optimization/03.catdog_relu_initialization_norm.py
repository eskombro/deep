...
def calculate_cost(cache, Y):
	cost = (-1/Y.shape[1]) * np.sum(np.dot(Y, np.log(cache["A2"]).T) + np.dot((1 - Y), np.log(1 - cache["A2"]).T))
	return(cost)
...
def training(X, iterations, learning_rate):
	...
	for i in range(0, iterations):
		...
		cost = calculate_cost(cache, Y)
		costs.append(cost)
		cache_dev = forward_propagation(X_test, params)
		cost_dev = calculate_cost(cache_dev, Y_test)
...
mean = np.mean(X, axis = 1).T.reshape(X.shape[0], 1)
variance = np.mean((X - mean)**2, axis = 1).T.reshape(X.shape[0], 1)
X = normalization(X, mean, variance)
X_test = normalization(X_test, mean, variance)
n_x = X.shape[0]
n_y = Y.shape[0]
